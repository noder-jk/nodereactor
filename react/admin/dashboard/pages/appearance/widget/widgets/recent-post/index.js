"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.RecentPostOutput = exports.RecentPostInput = void 0;

var _react = _interopRequireDefault(require("react"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { "default": obj }; }

var RecentPostInput = function RecentPostInput(props) {
  return _react["default"].createElement("div", null, "Sorry, this 'Recent Post' widget is unavailable in this NR version.");
};

exports.RecentPostInput = RecentPostInput;

var RecentPostOutput = function RecentPostOutput() {
  return _react["default"].createElement("p", null, "Sorry, this 'Recent Post' widget is unavailable in this NR version.");
};

exports.RecentPostOutput = RecentPostOutput;