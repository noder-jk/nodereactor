...
class MenuProcessor extends Component
{
    constructor(props)
    {
        super(props);
    ...
    render()
    {
        ...
        let {response, properties}=this.props;
        ...
        let recurs=(menu)=>
        {
            ...
            else if(typeof menu=='object')
            {
                return  <li key={menu.key} className={li_class}>
                            <a href={menu.url} className={anchor_class + (window.location.pathname==menu.url ? li_active_class : '')} onClick={this.handleClick}>{menu.title}</a>
...
const nav_menu=(props)=>
{
    ...
    {
        return <MenuProcessor response={window.nr_menu_list} properties={props}/>
    }

    return <Placeholder action="nr_get_menu_for_visitor" component={MenuProcessor} properties={props}/>
}
